...
export class ShareClient {
  ...
  private async connect(generation: number): Promise<void> {
    ...
    const finishAck = (
      ...
      this.pendingPayload = null;
      if (!accepted || !this.sendImmediate({ t: "ack", nonce })) return;
    ...
    const acknowledgePayload = (
      ...
      batch.nonce = nonce;
      if (typeof batch.accepted === "boolean") {
        finishAck(batch, nonce, batch.accepted);
        return;
      }
      void batch.accepted.then(
        (accepted) => finishAck(batch, nonce, accepted),
        () => finishAck(batch, nonce, false),
      );
    };
